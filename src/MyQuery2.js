...
const MY_QUERY2_QUERY = graphql`
  query MyQuery2_MyQuery2Query {
    me {
      serviceMetadata {
        loggedInServices {
          bearerToken
...
export function MyQuery2Query(props) {
  const data = useLazyLoadQuery(MY_QUERY2_QUERY, props, {
  ...
  const dataEl = data ? (
    <div className="data-box">
      <h3>
        Data for MyQuery2 <LocationNote />
      </h3>
...
export default function MyQuery2QueryForm(props) {
  ...
  return (
    <div>
      ...
      >
        <Suspense fallback={"Loading MyQuery2Query..."}>
          <MyQuery2Query {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
