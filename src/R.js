...
export function RQuery(props) {
  const data = useLazyLoadQuery(R_QUERY, props, {
...
export default function RQueryForm(props) {
  ...
  return (
    <div>
      ...
      >
        <Suspense fallback={"Loading RQuery..."}>
          <RQuery {...queryVariables} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
